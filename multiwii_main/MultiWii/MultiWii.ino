  ...
  if (currentTime > rcTime ) { // 50Hz
    ...
    if(rcDelayCommand == 20) {
      if(f.ARMED) {                   // actions during armed
        ...
        if (conf.activate[BOXARM] == 0 /*&& rcSticks == THR_LO + YAW_LO + PIT_CE + ROL_CE*/) {/*blinkLED(2,255,1);*/
          go_disarm();
        }    // Disarm via YAW
#endif
        ...
        else if (conf.activate[BOXARM] == 0 /*&& rcSticks == THR_LO + YAW_HI + PIT_CE + ROL_CE*/) {/*blinkLED(2,255,1);*/
          go_arm();
        }      // Arm via YAW
#endif
#ifdef ALLOW_ARM_DISARM_VIA_TX_ROLL
