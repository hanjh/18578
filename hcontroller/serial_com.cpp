...
void readFController(int fd, fcontroller_data_t* data)
{
    ...
    while(1)
    {
        ...
        int n = 0;
        int i = 0;
        // maybe change to while loop later 
        for(i = 0; i < sizeof(fcontroller_data_t); i += n)
        {
            // attempt to read all bytes at a time, read again if necessary
            n = read(fd, ((char*)data) + i, sizeof(data) - i);
        }

        // check that the data is okay
        if (data->header == (data->rotation[0] ^ data->rotation[1]))
        {
            if (data->footer == (data->rVelocity[0] ^ data->rVelocity[1]))
            {   
